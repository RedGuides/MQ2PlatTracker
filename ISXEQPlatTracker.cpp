...
#include "ISXEQPlatTracker.h"

// The mandatory pre-setup function.  Our name is "ISXEQPlatTracker", and the class is ISXEQPlatTracker.
...
// our instanced class.
ISXPreSetup("ISXEQPlatTracker",ISXEQPlatTracker);
...
unsigned int ISXEQPlatTrackerXML=0;
...
bool ISXEQPlatTracker::Initialize(ISInterface *p_ISInterface)
...
WriteChatf("ISXEQPlatTracker Loaded");
return true;
}

// shutdown sequence
void ISXEQPlatTracker::Shutdown()
{
    // save settings, if you changed them and want to save them now.  You should normally save
    // changes immediately.
    //pISInterface->ExportSet(ISXEQPlatTrackerXML,XMLFileName);

    pISInterface->UnloadSet(ISXEQPlatTrackerXML);
...
void ISXEQPlatTracker::ConnectServices()
{
...
void ISXEQPlatTracker::RegisterCommands()
{
    // add any commands
...
void ISXEQPlatTracker::RegisterAliases()
{
    // add any aliases
}

void ISXEQPlatTracker::RegisterDataTypes()
{
    // add any datatypes
    // pMyType = new MyType;
...
void ISXEQPlatTracker::UnRegisterCommands()
{
    // remove commands
    //    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQPlatTracker::UnRegisterAliases()
{
    // remove aliases
}
void ISXEQPlatTracker::UnRegisterDataTypes()
{
    // remove data types
    //if (pMyType)
    //{
...
void ISXEQPlatTracker::UnRegisterTopLevelObjects()
{
    // remove Top-Level Objects
    //    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQPlatTracker::UnRegisterServices()
{
    // shutdown our own services
    //    if (hISXEQPlatTrackerService)
    //        pISInterface->ShutdownService(this,hISXEQPlatTrackerService);
}

void ISXEQPlatTracker::LoadSettings()
{
    bool bChanged=false;

    // open the settings file, and store the ID number in ISXEQPlatTrackerXML
    ISXEQPlatTrackerXML=pISInterface->OpenSettings(XMLFileName);
    ...
    Any set can be referred to by its ID.  The base set is stored in ISXEQPlatTrackerXML.  Note that the settings
    ...
    */
    ...
    if (unsigned int GeneralGUID=pISInterface->FindSet(ISXEQPlatTrackerXML,"General"))
    {
